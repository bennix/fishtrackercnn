...
void mexFunction(int nout, mxArray *out[],
    ...
    if(s==1){
        ...
        for(j=0;j<N;j++){
            for(i=s;i<M-2;i++){
                AT(result,i,j) = (AT(P_pt,i-2,j) + AT(P_pt,i+2,j) - 2*AT(P_pt,i,j) ) /4;
            }
        }
        //top
        ...
        for(j=0;j<N;j++){
            for(i=0;i<2;i++){
                AT(result,i,j) = (AT(P_pt,i+2,j) + AT(P_pt,i,j) - 2*AT(P_pt,i+1,j));
            }
        }
        //bottom
        ...
        for(j=0;j<N;j++){
            for(i=s;i<M-s;i++){
                AT(result,i,j) = (AT(P_pt,i-s,j) + AT(P_pt,i+s,j) - 2*AT(P_pt,i,j) ) /s/s;
            }
        }
        //top
        ...
        for(j=0;j<N;j++){
            for(i=0;i<s;i++){
                AT(result,i,j) = (AT(P_pt,i+2*s,j) + AT(P_pt,i,j) - 2*AT(P_pt,i+s,j))/s/s;
            }
        }
        //bottom
